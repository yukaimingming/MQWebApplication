...
public class MQController : ControllerBase
{
    ...
    public IActionResult ComposePutAndGetMsgks([FromBody] PutMsgDto data)
    {
        ...
        {
            _logger.LogError(ex, "❌ 发送科室异常");
            return BadRequest(new { code = "-1", msg = "❌ 发送科室异常" });
        }
    ...
    public IActionResult ComposePutAndGetMsgbq([FromBody] PutMsgDto data)
    {
        ...
        try
        {
            ...
            //如果ds没有数据就返回
            if (ds.Tables.Count <= 0 && ds.Tables[1].Rows.Count <= 0) return Ok(new { code = "-1", msg = "❌ 无病区数据" });
        ...
        {
            _logger.LogError(ex, "❌ 发送病区异常");
            return BadRequest(new { code = "-1", msg = "❌ 发送病区异常" });
        }
    ...
    public IActionResult ComposePutAndGetMsgry([FromBody] PutMsgDto data)
    {
        ...
        {
            _logger.LogError(ex, "❌ 发送人员异常");
            return BadRequest(new { code = "-1", msg = "❌ 发送人员异常" });
        }
    }
}
